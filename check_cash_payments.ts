...
async function test() {
  const sql = "SELECT id, created_at, date, payment_method, amount, status FROM payments WHERE payment_method = 'Cash' ORDER BY created_at DESC LIMIT 5;";
  const { data, error } = await supabase.rpc('exec_sql', { sql_query: sql });
  console.log('Cash Payments:', data, error);
}
test();
