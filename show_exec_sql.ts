...
async function run() {
  const { data } = await supabase.rpc("exec_sql", { sql_query: "SELECT pg_get_functiondef(p.oid) FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid WHERE n.nspname = public AND p.proname = exec_sql;" })
  console.log(data);
}
run();
