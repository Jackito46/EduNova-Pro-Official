...
const CourseSignatureView: React.FC<{ user: UserProfile }> = ({ user }) => {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white p-4 sm:p-5 rounded-2xl shadow-lg border border-slate-800 relative z-20">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-start sm:items-center gap-3 min-w-0">
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-indigo-500/20 border border-indigo-400/30 flex items-center justify-center text-indigo-300 shadow-inner shrink-0 mt-0.5 sm:mt-0">
              <PenTool className="w-5 h-5 sm:w-5.5 sm:h-5.5" />
            </div>
            ...
            <div className="space-y-0.5 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-[11px] font-extrabold shrink-0 ${schoolCategoryBadge.color}`}>
                  <schoolCategoryBadge.icon size={13} className="shrink-0" />
                  <span>{schoolCategoryBadge.label}</span>
                </span>
              </div>
              
              <p className="text-slate-300 text-xs font-normal leading-relaxed max-w-3xl line-clamp-2 sm:line-clamp-none">
                {terms.subtitle}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap self-start lg:self-center w-full lg:w-auto justify-start lg:justify-end">
            ...
            {campuses && campuses.length > 0 && (
              <div className="min-w-0 max-w-full">
                <SelectPill
                  options={[
                    { value: 'ALL', label: 'Tous les Campus / Annexes', icon: Building2 },
            ...
            {isAdmin && (
              <div className="min-w-0 max-w-full">
                <SelectPill
                  options={[
                    { value: '', label: '-- Mes cours personnels --', icon: ShieldCheck },
                    ...allStaff.map(s => {
                      const campus = getCampusName(s.campus_id);
                      return {
                        value: s.id,
                        label: `${formatStudentName(s.last_name, s.first_name).fullName}${campus ? ` (${campus})` : ''}`,
                        icon: UserCheck
                      };
                    })
                  ]}
                  ...
                  className="text-xs max-w-full sm:max-w-[260px]"
                />
              </div>
            )}
          </div>
        ...
        <div className="mt-3 pt-2.5 border-t border-white/10 flex items-center justify-between gap-2.5 text-xs flex-wrap">
          <div className="flex items-center gap-2 text-slate-300">
            <div className="w-5 h-5 rounded-full bg-indigo-500/30 border border-indigo-400/40 flex items-center justify-center font-black text-[9px] text-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200/90 shadow-2xs flex items-center gap-2.5 sm:gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{terms.title.includes('Universitaire') ? 'Séances Signées' : 'Cours Émargés'}</p>
            <p className="text-base sm:text-lg font-black text-slate-900 mt-0.5">{kpis.totalSessions}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200/90 shadow-2xs flex items-center gap-2.5 sm:gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Volume Horaire</p>
            <p className="text-base sm:text-lg font-black text-slate-900 mt-0.5">{kpis.totalHours} <span className="text-xs font-bold text-slate-500">heures</span></p>
          </div>
        </div>
        ...
        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200/90 shadow-2xs flex items-center gap-2.5 sm:gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Validation Admin</p>
            <p className="text-base sm:text-lg font-black text-slate-900 mt-0.5">{kpis.validatedSessions} <span className="text-xs font-bold text-slate-400">/ {kpis.totalSessions}</span></p>
          </div>
        </div>

        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200/90 shadow-2xs flex items-center gap-2.5 sm:gap-3">
          <div className="w-9 h-9 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center font-bold shrink-0 border border-purple-100">
            <Users size={16} />
          </div>
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Taux de Validation</p>
            <p className="text-base sm:text-lg font-black text-slate-900 mt-0.5">{kpis.avgAttendancePercent}%</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 sm:gap-4 items-start">
        ...
        <div className="lg:col-span-5 space-y-3.5">
          <div className="bg-white rounded-2xl shadow-2xs border border-slate-200/90 p-3.5 sm:p-4 space-y-3">
            
            {/* Ergonomic Date Navigator (Harmonisé DatePickerPill) */}
            ...
            {loading ? (
              ...
              <div className="py-6 text-center bg-slate-50/70 rounded-xl border border-dashed border-slate-200 px-3 space-y-1.5">
                <div className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-500 flex items-center justify-center mx-auto border border-indigo-100">
                ...
                {todayAssignments.map((assignment) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-2.5 sm:p-3 rounded-xl border transition-all relative group flex items-start justify-between cursor-pointer ${
                        isSelected 
                          ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-600/20 ring-2 ring-indigo-600' 
                          : alreadySigned
                          ? 'bg-emerald-50/70 border-emerald-200 hover:border-emerald-300 text-slate-900'
                          : 'bg-white border-slate-200 hover:border-indigo-300 text-slate-900 hover:shadow-2xs'
                      }`}
                    >
                      <div className="space-y-1 min-w-0 pr-2">
                        ...
                        <div className={`flex items-center gap-2 text-[10px] sm:text-[11px] ${isSelected ? 'text-indigo-200' : 'text-slate-500'}`}>
        ...
        <div className="lg:col-span-7 space-y-3.5">
          <div className="bg-white rounded-2xl shadow-2xs border border-slate-200/90 overflow-hidden">
            <div className="px-3.5 py-2.5 sm:px-4 sm:py-3 border-b border-slate-100 bg-slate-50/70 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-900 text-xs sm:text-sm">{terms.formTitle}</h3>
                  <p className="text-[11px] text-slate-500">{terms.formDesc}</p>
                </div>
              </div>
            </div>

            {selectedAssignment ? (
              <form onSubmit={handleSubmitSignature} className="p-3.5 sm:p-4 space-y-3.5">
                
                {/* Selected Course Recap Card */}
                <div className="p-3 bg-indigo-50/80 rounded-xl border border-indigo-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-extrabold uppercase text-indigo-700 bg-indigo-100/70 px-1.5 py-0.5 rounded">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-2.5 sm:p-3 text-xs font-medium text-slate-900 bg-slate-50/60 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 leading-relaxed"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 text-xs font-medium text-slate-900 bg-slate-50/60 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400"
                  />
                </div>
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <h5 className="font-extrabold text-slate-900 text-xs flex items-center gap-1.5">
                        <Users size={14} className="text-indigo-600" /> Contrôle des Présences {isUniversity ? 'Étudiants' : isProfessional ? 'Stagiaires' : 'Élèves'}
                      </h5>
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                      <span className="text-xs font-black text-indigo-700 bg-indigo-100 px-2.5 py-0.5 rounded-lg border border-indigo-200/60">
                      ...
                      {students.length > 0 && (
                        <button
                          ...
                          className="px-2.5 py-1 text-xs font-bold bg-white text-indigo-600 hover:bg-indigo-50 border border-slate-200 rounded-lg transition-all shadow-2xs cursor-pointer"
                        >
                  ...
                  {showAttendance && students.length > 0 && (
                    <div className="pt-2 border-t border-slate-200 space-y-2">
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div className="relative flex-1 min-w-[140px]">
                          ...
                          <input
                            ...
                            className="w-full pl-7 pr-2.5 py-1 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                        </div>
                        ...
                        <div className="flex items-center gap-1 text-xs">
                          <button
                            ...
                            className="px-2 py-0.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-md text-[10px] border border-emerald-200 cursor-pointer"
                          >
                            Tous Présents
                          </button>
                          <button
                            ...
                            className="px-2 py-0.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold rounded-md text-[10px] border border-rose-200 cursor-pointer"
                          >
                            Tous Absents
                          </button>
                        </div>
                      ...
                      <div className="max-h-44 sm:max-h-48 overflow-y-auto pr-1 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                        {filteredStudents.map((st) => {
                          ...
                          return (
                            <button
                              ...
                              className={`p-2 rounded-lg border text-left flex items-center justify-between transition-all active:scale-98 cursor-pointer ${
                                isPresent 
                ...
                <div className="p-2.5 bg-indigo-50/50 rounded-xl border border-indigo-100 flex items-center gap-2 text-slate-600 text-[11px]">
              ...
              <div className="py-10 sm:py-12 text-center px-4 space-y-2">
                <div className="w-12 h-12 bg-indigo-50 text-indigo-500 rounded-xl flex items-center justify-center mx-auto border border-indigo-100 shadow-2xs">
                  <PenTool className="w-6 h-6" />
                </div>
                <h3 className="text-sm sm:text-base font-extrabold text-slate-900">Sélectionnez une séance pour commencer</h3>
          ...
          <div className="bg-white rounded-2xl shadow-2xs border border-slate-200/90 overflow-hidden">
            <div className="px-3.5 py-2.5 sm:px-4 sm:py-3 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
              <div className="flex items-center gap-2">
                <History className="w-4 h-4 text-indigo-600" />
                <div>
                  <h3 className="font-extrabold text-slate-900 text-xs sm:text-sm">Historique des Émargements Récents</h3>
                  <p className="text-[10px] sm:text-[11px] text-slate-500">Traçabilité complète des cours signés et validés.</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200 text-xs">
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-md font-bold text-[10px] sm:text-[11px] transition-all cursor-pointer ${
                      historyFilter === 'ALL' ? 'bg-white text-slate-900 shadow-2xs' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-md font-bold text-[10px] sm:text-[11px] transition-all cursor-pointer ${
                      historyFilter === 'VALIDATED' ? 'bg-emerald-600 text-white shadow-2xs' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                    Validés
                  </button>
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-md font-bold text-[10px] sm:text-[11px] transition-all cursor-pointer ${
                      historyFilter === 'SIGNED' ? 'bg-amber-600 text-white shadow-2xs' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                    En Attente
                  </button>
                ...
                <div className="relative min-w-[120px] sm:min-w-[140px]">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-2.5 py-1 text-xs font-medium bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500"
                  />
                </div>
              </div>
            </div>
              ...
              {filteredHistory.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {filteredHistory.map((sig) => {
                    ...
                    return (
                      <div key={sig.id} className="p-3 sm:py-2.5 sm:px-4 hover:bg-slate-50/80 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                        <div className="space-y-0.5 min-w-0 pr-2">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-[10px] font-black text-indigo-700 uppercase tracking-wider bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100">
                              {sig.class_name}
                            </span>
                        ...
                        <div className="shrink-0 flex items-center gap-2 self-start sm:self-auto">
                          <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-lg border ${
                            ...
                            {sig.signature_status === 'VALIDATED' ? <CheckCircle2 size={11} /> : <Clock size={11} />}
                            {sig.signature_status === 'VALIDATED' ? 'Validé' : 'En Attente'}
                          </span>
                        </div>
                      </div>
                    );
