...
const CommunicationHistory: React.FC<CommunicationHistoryProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-3.5 sm:space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 items-stretch sm:items-center justify-between bg-white p-3 sm:p-3.5 rounded-2xl border border-slate-200 shadow-xs">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 outline-none transition-all text-xs sm:text-sm font-medium"
          />
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <Filter className="text-slate-400 shrink-0 hidden sm:block" size={16} />
          <SelectPill
            options={[
              { value: 'all', label: 'Tous les canaux' },
              { value: 'email', label: 'Emails uniquement' },
