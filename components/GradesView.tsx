  ...
  return (
    <div className="space-y-3.5 max-w-7xl mx-auto pb-16 px-2 sm:px-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        ...
        <div 
          className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200/90 shadow-xs flex items-center gap-2.5"
          title={`${kpiStats.filledCount} note(s) saisie(s) sur un total de ${kpiStats.totalCells} attendue(s) (${kpiStats.studentCount} élèves × ${activeColumns.length} matière(s))`}
        >
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            kpiStats.percentageFilled === 100 
          ...
          <div className="min-w-0">
            <p className="text-[10px] font-bold text-slate-400 uppercase truncate">Complétude Saisie</p>
            <p className="text-xs sm:text-sm font-bold text-slate-800 truncate flex items-center gap-1">
              <span>{kpiStats.filledCount} / {kpiStats.totalCells}</span>
              <span className={`text-[11px] font-semibold ${
                kpiStats.percentageFilled === 100 ? "text-emerald-600" : "text-amber-600"
              }`}>
                ({kpiStats.percentageFilled}%)
              </span>
            </p>
            <p className="text-[10px] text-slate-400 truncate hidden sm:block">
              {kpiStats.studentCount} élv × {activeColumns.length} mat.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200/90 shadow-xs flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
          ...
          <div className="hidden md:flex items-center gap-0.5 bg-slate-50 border border-slate-200/90 rounded-xl p-1 text-[11px] font-medium text-slate-600 shrink-0 shadow-2xs">
          ...
          <div className="hidden lg:flex items-center gap-0.5 bg-slate-50 border border-slate-200/90 rounded-xl p-1 text-[11px] font-medium text-slate-600 shrink-0 shadow-2xs">
        ...
        <div className="flex items-center gap-2 shrink-0 ml-auto">
          ...
          {!isReadOnly && (
            <button
              ...
              title="Enregistrer et sceller les notes (Ctrl+S)"
            >
              {saving ? (
                <>
