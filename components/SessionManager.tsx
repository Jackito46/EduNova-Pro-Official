...
export default function SessionManager({ user, schoolData, years, onRefresh }: SessionManagerProps) {
  ...
  return (
    <div id="session-manager-root" className="space-y-3 sm:space-y-4">
      
      {/* 1. Header & Context Infotip */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="space-y-0.5">
          ...
          <h2 className="text-base sm:text-lg md:text-xl font-black tracking-tight text-slate-900">
            Gestion des {terminology.academicYears}
          </h2>
          <p className="text-slate-600 text-xs font-semibold max-w-xl leading-snug">
            {isHigherEd 
        ...
        {isHigherEd ? (
          <button 
            ...
            className="px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 transition-all active:scale-95 flex items-center gap-1.5 whitespace-nowrap self-start sm:self-center cursor-pointer"
          >
      ...
      {/* 2. Metrics Grid */}
      <div className={`grid grid-cols-1 ${isHigherEd ? 'sm:grid-cols-2 lg:grid-cols-4' : 'sm:grid-cols-2'} gap-2.5 sm:gap-3`}>
        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
          <div className="space-y-0.5">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Total {terminology.academicYears}</p>
            <p className="text-xl sm:text-2xl font-black text-slate-900">{totalSessionsCount}</p>
          </div>
        ...
        <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
          <div className="space-y-0.5">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">{terminology.academicYears} Actives</p>
        ...
        {isHigherEd && (
          <>
            <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
              <div className="space-y-0.5">
                <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Sessions Intensives</p>
                <p className="text-xl sm:text-2xl font-black text-amber-600">{intensiveCount}</p>
              </div>
            ...
            <div className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
              <div className="space-y-0.5">
                <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Sessions Spéciales</p>
                <p className="text-xl sm:text-2xl font-black text-purple-600">{specialCount}</p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {templates.map(tpl => {
                ...
                return (
                  ...
                  >
                    <span className={`text-[9px] font-black uppercase tracking-wider ${isSelected ? 'text-indigo-100' : 'text-slate-600'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <AnimatePresence mode="popLayout">
          {filteredYears.length === 0 ? (
            <div className="md:col-span-2 py-10 text-center bg-slate-50 border border-dashed border-slate-200 rounded-2xl space-y-2">
              {viewMode === 'ARCHIVED' ? (
                <>
                  <Bookmark size={32} className="mx-auto text-slate-300" />
                  <p className="text-slate-700 text-xs sm:text-sm font-bold">Aucune archive disponible.</p>
                ...
                <>
                  ...
                  <p className="text-slate-700 text-xs sm:text-sm font-bold">Aucune session active ou en préparation trouvée.</p>
                  <p className="text-slate-500 text-[11px]">Créez une nouvelle session pour démarrer.</p>
                </>
              )}
            </div>
          ) : (
