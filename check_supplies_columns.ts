...
async function checkColumnExistence() {
  const { error: err1 } = await supabase.from('school_supplies').select('payment_method').limit(1);
  console.log('school_supplies.payment_method exists:', !err1 || err1.code !== 'PGRST204', err1?.message);
}
