...
async function test() {
  const sql = "SELECT id, created_at, payment_method, total_amount, amount_htg_equivalent FROM school_supplies ORDER BY created_at DESC LIMIT 5;";
  const { data, error } = await supabase.rpc('exec_sql', { sql_query: sql });
  console.log('Supplies:', data, error);
}
test();
