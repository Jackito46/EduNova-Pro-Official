...
-- 2. Ensure functions are plpgsql to prevent inlining
...
CREATE POLICY "Profiles read superadmin" ON public.profiles FOR SELECT USING (public.is_super_admin_safe(auth.uid()));
CREATE POLICY "Profiles read school" ON public.profiles FOR SELECT USING (school_id = public.get_user_school_id(auth.uid()));
CREATE POLICY "Profiles update school admin" ON public.profiles FOR UPDATE USING (
    EXISTS (
...
CREATE POLICY "Schools read own" ON public.schools FOR SELECT USING (id = public.get_user_school_id(auth.uid()));
CREATE POLICY "Schools read superadmin" ON public.schools FOR SELECT USING (public.is_super_admin_safe(auth.uid()));
CREATE POLICY "Schools update superadmin" ON public.schools FOR UPDATE USING (public.is_super_admin_safe(auth.uid()));
