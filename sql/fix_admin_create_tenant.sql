-- Consolidate admin_create_tenant functions

-- 4-arg version (Auto-generate school_id)
...
BEGIN
    ...
    -- 7. Create profile (Trigger handle_new_user should handle it, but we insert directly for robustness)
...
-- 5-arg version (Manual school_id)
CREATE OR REPLACE FUNCTION public.admin_create_tenant(
    p_school_name TEXT,
    p_school_id TEXT, -- Note: accepting TEXT to avoid casting issues in call
    p_admin_email TEXT,
    p_admin_password TEXT,
    p_admin_name TEXT
)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.schools WHERE id = v_school_id) THEN
        RETURN jsonb_build_object('success', false, 'error', 'Cet identifiant d''établissement existe déjà.');
    END IF;
