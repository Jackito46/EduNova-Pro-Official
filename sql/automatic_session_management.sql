-- ==========================================================
-- AUTOMATIC & ADAPTIVE SESSION MANAGEMENT
-- ==========================================================

-- 1. Helper function to determine the default academic session config based on school type
CREATE OR REPLACE FUNCTION public.get_default_academic_session_config(p_school_type TEXT DEFAULT 'CLASSIC')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    ...
    v_end_date TEXT;
BEGIN
    -- Try to get from global settings first (Super Admin override)
    SELECT value INTO v_config FROM public.global_settings WHERE key = 'default_session_config';
    
    -- If manual configuration is forced by Super Admin, return it
    IF v_config IS NOT NULL AND v_config->>'mode' = 'manual' THEN
        RETURN v_config;
    END IF;

    -- Fallback/Auto dynamic calculation
    v_current_month := EXTRACT(MONTH FROM now()); -- 1 to 12
    v_current_year := EXTRACT(YEAR FROM now());

    -- Pre-Enrollment & Onboarding Window Logic:
    -- From MAY (month 5) to DECEMBER, schools creating an account are preparing for the UPCOMING academic year (current_year - current_year+1).
    -- e.g. In May/June/July/August 2026, newly created schools receive 2026-2027 to start admissions & registrations!
    -- From JANUARY to APRIL (months 1-4), mid-year onboardings join the ongoing year (current_year-1 - current_year).
    ...
    IF p_school_type = 'UNIVERSITY' THEN
        -- University cycle: Octobre - Juillet
        v_start_date := v_start_year::TEXT || '-10-01';
        v_end_date := v_end_year::TEXT || '-07-31';
    ELSE
        -- Classic/Professional cycle: Septembre - Juin
        v_start_date := v_start_year::TEXT || '-09-01';
        v_end_date := v_end_year::TEXT || '-06-30';
    END IF;
...
-- 2. Enhanced seed_school_data that uses the smart adaptive academic session config
...
BEGIN
    ...
    -- Obtenir la config de session par défaut intelligente
    v_session_config := public.get_default_academic_session_config(v_school_type);

    -- Création d'année académique par défaut adaptative
    ...
    IF v_academic_year_id IS NULL THEN
        ...
        VALUES (
            ...
            true, 
            'ACTIVE', 
            (v_session_config->>'start_date')::DATE, 
            (v_session_config->>'end_date')::DATE
        )
        ON CONFLICT (school_id, label) DO NOTHING RETURNING id INTO v_academic_year_id;
        
        IF v_academic_year_id IS NULL THEN
            SELECT id INTO v_academic_year_id FROM public.academic_years WHERE school_id = p_school_id AND label = (v_session_config->>'label') LIMIT 1;
        END IF;
    END IF;
    ...
    ELSIF v_school_type = 'PROFESSIONAL' THEN
        ...
        PERFORM public.upsert_class(p_school_id, 'Informatique de Bureau I', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Informatique de Bureau II', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Art Culinaire I', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Art Culinaire II', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Plomberie', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Électricité Résidentielle', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Mécanique Auto I', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Mécanique Auto II', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Esthétique et Cosmétologie', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Coupe et Couture I', 'CERTIFICAT');
        PERFORM public.upsert_class(p_school_id, 'Coupe et Couture II', 'CERTIFICAT');
        ...
        -- Art Culinaire
        FOR v_class_id IN SELECT id FROM public.classes WHERE school_id = p_school_id AND name LIKE 'Art Culinaire%' LOOP
        ...
        -- Couture
        FOR v_class_id IN SELECT id FROM public.classes WHERE school_id = p_school_id AND name LIKE 'Coupe%' LOOP
