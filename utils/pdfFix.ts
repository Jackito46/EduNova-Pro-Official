/**
 * Utility to fix html2canvas issues with modern color functions (OKLCH, OKLAB, color-mix) used in Tailwind CSS v4.
 * It translates known Tailwind classes to explicit inline styles (resolved to hex/rgba),
 * replaces modern color functions in <style> tags with compatible sRGB hex/rgb values,
 * and guarantees text and borders remain completely visible and crisp.
 */
...
export const fixOklchForCanvas = (clonedDoc: Document) => {
  ...
  const processCssText = (css: string) => {
    let result = css.replace(/\/\*[\s\S]*?\*\//g, '');
    result = result.replace(/@import\s+[^;]+;/gi, '');

    // Map known palette color variables directly: --color-slate-900: oklch(...) -> #0f172a
    Object.entries(PALETTE).forEach(([family, shades]) => {
      Object.entries(shades).forEach(([shade, hex]) => {
        const varRegex = new RegExp(`(--color-${family}-${shade}\\s*:\\s*)(?:oklch|oklab|color-mix)\\([^;]+\\)`, 'gi');
        result = result.replace(varRegex, `$1${hex}`);
      });
    });

    // Fallback modern functions in specific properties to visible solid colors instead of transparent
    result = result.replace(/(color\s*:\s*)(?:color-mix|oklch|oklab)\s*\((?:[^()]+|\((?:[^()]+|\((?:[^()]+|\([^()]*\))*\))*\))*\)/gi, '$1#0f172a');
    result = result.replace(/(background(?:-color)?\s*:\s*)(?:color-mix|oklch|oklab)\s*\((?:[^()]+|\((?:[^()]+|\((?:[^()]+|\([^()]*\))*\))*\))*\)/gi, '$1#ffffff');
    result = result.replace(/(border(?:-color)?\s*:\s*)(?:color-mix|oklch|oklab)\s*\((?:[^()]+|\((?:[^()]+|\((?:[^()]+|\([^()]*\))*\))*\))*\)/gi, '$1#e2e8f0');

    // General fallback: use currentColor for text/stroke/fill, and #e2e8f0 for others to prevent canvas crashes
    result = result.replace(colorMixRegex, 'currentColor');
    result = result.replace(colorRegex, '#0f172a');
    return result;
  };
  ...
  try {
    ...
    allElements.forEach(el => {
      ...
      if (styleAttr && (styleAttr.includes('oklch') || styleAttr.includes('oklab') || styleAttr.includes('color-mix'))) {
        el.setAttribute('style', processCssText(styleAttr));
      }
    ...
    // Ensure all image elements in clone have crossOrigin to avoid canvas tainting
    const allImgs = Array.from(clonedDoc.getElementsByTagName('img'));
    allImgs.forEach(img => {
      try {
    ...
    // 2. Safely replace unsupported modern color functions in <style> tags with valid hex/color values
    ...
    styleTags.forEach(style => {
      try {
        if (style.textContent && (style.textContent.includes('oklch') || style.textContent.includes('oklab') || style.textContent.includes('color-mix'))) {
          style.textContent = processCssText(style.textContent);
        }
      } catch (e) {}
    });
    ...
    // 4. Force font metrics stability, default visible high-contrast color and overflow safety
    if (clonedDoc.body) {
