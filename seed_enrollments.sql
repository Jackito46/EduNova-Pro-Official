CREATE OR REPLACE FUNCTION pg_temp.seed_records() RETURNS JSONB AS $$
DECLARE
    v_school_id UUID;
...
BEGIN
    RAISE NOTICE 'Starting enrollment injection...';
    ...
    RETURN jsonb_build_object('status', 'success', 'inserted', total_inserted);
END;
$$ LANGUAGE plpgsql;

SELECT pg_temp.seed_records();
